...
export default function KolmaPOSPremium() {
  ...
  // Estadísticas del día (Control de dinero y piezas)
  ...
  const handleCheckout = () => {
    setShiftStats(prev => ({
      totalSales: prev.totalSales + currentTotal,
      itemsSold: prev.itemsSold + currentItems
    }));
    setTicket([]);
  ...
  const resetShift = () => {
    setShiftStats({ totalSales: 0, itemsSold: 0 });
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-[#F6F6F7]">
      <div className="text-center">
        ...
        <p className="text-[#202223] font-black italic uppercase text-[10px] tracking-widest">Conectando Shopify</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-[#F6F6F7] text-[#202223] font-sans overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-[#E1E3E5] flex items-center px-4 md:px-6 gap-4 shadow-sm sticky top-0 z-40">
          ...
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className="w-full h-10 md:h-11 pl-10 md:pl-12 pr-4 bg-[#F1F2F3] border-none rounded-lg focus:ring-2 focus:ring-[#008060] font-medium outline-none text-sm"
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-emerald-100 text-[#008060] rounded-full border border-emerald-200">
               ...
               <span className="text-[10px] font-black uppercase tracking-widest">Shopify Activo</span>
            </div>
          </div>
      ...
      <aside className="w-full sm:w-[400px] lg:w-[420px] bg-white border-l border-[#E1E3E5] hidden xl:flex flex-col">
        <div className="h-16 px-6 border-b border-[#E1E3E5] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-gray-100 text-gray-600 text-[10px] px-2.5 py-1 rounded-md font-bold">{currentItems} piezas</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 custom-scrollbar">
          {ticket.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center scale-90">
              ...
              <p className="font-black text-[10px] uppercase tracking-widest text-gray-500 leading-relaxed text-center">Selecciona productos<br/>para empezar</p>
            </div>
          ) : ticket.map(item => (
        ...
        <div className="p-6 bg-[#F9FAFB] border-t border-[#E1E3E5]">
          ...
          <button 
            ...
            className={`w-full py-5 rounded-2xl font-black text-xl uppercase tracking-widest transition-all shadow-xl active:scale-95 ${currentTotal > 0 ? "bg-[#008060] text-white shadow-emerald-200" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          >
            Cobrar Ahora
          </button>
        </div>
      ...
      {showShiftModal && (
        <div className="fixed inset-0 bg-[#1A1C1D]/90 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 text-center animate-in zoom-in-95">
              ...
              <h2 className="text-2xl font-black uppercase italic tracking-tighter text-gray-900 mb-8">Corte de Caja</h2>
              ...
              <div className="space-y-3">
                <button onClick={resetShift} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-orange-200 active:scale-95">Confirmar y Reiniciar</button>
                <button onClick={() => setShowShiftModal(false)} className="w-full py-4 bg-gray-100 text-gray-500 rounded-2xl font-black uppercase tracking-widest active:scale-95">Seguir Vendiendo</button>
              </div>
          </div>
        </div>
      )}
...
function MenuBtn({ active, onClick, label, open, icon }) {
  return (
    <button 
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-3.5 py-3 rounded-xl transition-all ${active ? "bg-[#008060] text-white shadow-lg translate-x-1" : "text-gray-400 hover:bg-gray-800 hover:text-white"}`}
    >
