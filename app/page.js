...
.join('%0A');
​const mensaje = 'NUEVO PEDIDO - XIIAO KITCHEN%0A%0ACliente: ' + nombreCliente + '%0ATelefono: ' + telefonoCliente + '%0A' + (tipoPedido === 'delivery' ? 'Direccion: ' + direccionCliente : 'Tipo: En Local') + '%0A%0AProductos:%0A' + listaProductos + '%0A%0ATotal: RD$' + total;
...
const mensaje = 'Hola ' + ord.cliente + '! Tu pedido en Xiiao Kitchen ya esta Preparado y listo para retirar en el local. ¡Te esperamos!';
...
<div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs bg-orange-500 text-black">OK</div>
...
​<div className="flex flex-col items-center">
<div className={w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${trackedOrder.estado !== 'Pendiente' ? 'bg-orange-500 text-black' : 'bg-gray-800 text-gray-500'}}>COOK</div>
...
{trackedOrder.tipo === 'delivery' ? 'DEL' : 'LIST'}
</div>
...
{trackedOrder.estado === 'Enviado' ? 'Tu pedido va en camino' : trackedOrder.estado === 'Preparado' ? 'Tu pedido esta preparado y listo para retirar en el local' : 'Estado actual: ' + trackedOrder.estado}
</p>
</div>
</div>
)}
...
<button onClick={() => setShowPinModal(true)} className="text-gray-700 hover:text-orange-500 transition-colors text-xl" title="Panel de Administracion">
PIN
</button>
...
<input type="text" value={nombreCliente} onChange={e => setNombreCliente(e.target.value)} required placeholder="Ej. Juan Perez" className="w-full bg-[#050505] border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-orange-500" />
</div>
<div>
<label className="text-xs font-bold text-gray-400 block mb-1">Tu Telefono / WhatsApp</label>
...
{tipoPedido === 'delivery' && (
<div>
<label className="text-xs font-bold text-gray-400 block mb-1">Direccion de Entrega en Cotui</label>
<textarea value={direccionCliente} onChange={e => setDireccionCliente(e.target.value)} rows="2" required placeholder="Calle, numero de casa, referencias..." className="w-full bg-[#050505] border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-orange-500 resize-none"></textarea>
</div>
)}
...
<div>
<h2 className="text-2xl font-black text-orange-500">Gestor Rapido</h2>
...
<p className="text-gray-400 text-xs">Gestiona ordenes en tiempo real</p>
</div>
...
<div className="space-y-4 pb-20">
{orders.length === 0 ? (
<div className="text-center py-20 text-gray-500 text-sm">No hay pedidos registrados todavia.</div>
) : (
orders.map(ord => (
...
<p className="text-xs text-gray-400 mb-1">Direccion: {ord.direccion}</p>
...
<span className="font-bold text-orange-400 bg-orange-500/10 px-3 py-1 rounded-full text-sm inline-block w-max border border-orange-500/25">{carrito.length} {carrito.length === 1 ? 'articulo' : 'articulos'}</span>
</div>
...
<button onClick={() => eliminarDelCarrito(idx)} className="text-red-400 hover:text-red-300 px-1 font-bold">x</button>
</div>
</div>
))}
</div>
...
<button onClick={() => setTipoPedido('local')} className={flex-1 py-3 text-sm font-bold rounded-xl transition-all ${tipoPedido === 'local' ? 'bg-[#1c1c1c] text-orange-500 border border-white/5' : 'text-gray-500'}}>En Local</button>
</div>
