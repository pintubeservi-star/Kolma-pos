...
export default function KolmaPOSPremium() {
  ...
  const resetShift = () => {
    ...
    alert("Turno cerrado. Caja reiniciada a cero.");
  };
      ...
      <aside className={`fixed inset-y-0 right-0 w-full sm:w-[400px] lg:w-[420px] xl:static bg-white border-l border-[#E1E3E5] flex flex-col z-[80] xl:z-10 transition-transform duration-300 ease-in-out transform ${isTicketOpen ? "translate-x-0" : "translate-x-full xl:translate-x-0"} shadow-2xl xl:shadow-none`}>
        ...
        <div className="p-6 bg-[#F9FAFB] border-t border-[#E1E3E5] shrink-0">
          ...
          <button 
            ...
            className={`w-full py-5 rounded-2xl font-black text-xl uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 ${
              currentTotalMoney > 0 
              ? "bg-[#008060] text-white hover:bg-[#006e52] shadow-lg" 
        ...
        <div className="fixed inset-0 bg-[#1A1C1D]/95 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 md:p-10 text-center">
              <div className="w-20 h-20 bg-orange-50 text-orange-600 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl shadow-inner border border-orange-100">
                <Icons.Power />
              </div>
              <h2 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter text-gray-900 mb-2">Corte de Caja</h2>
              <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-8">Resumen de Ventas de la Sesión</p>
              
              <div className="grid grid-cols-1 gap-4 mb-8">
                <div className="bg-[#F9FAFB] p-6 rounded-3xl border border-[#E1E3E5] flex flex-col items-start relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-50 rounded-bl-full -z-10"></div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Venta Acumulada</p>
                  <p className="text-4xl font-black text-[#008060] tracking-tighter">RD${shiftStats.totalSales.toLocaleString()}</p>
                </div>
                <div className="bg-[#F9FAFB] p-6 rounded-3xl border border-[#E1E3E5] flex flex-col items-start relative overflow-hidden">
                   <div className="absolute top-0 right-0 w-16 h-16 bg-blue-50 rounded-bl-full -z-10"></div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total de Piezas Salidas</p>
                  <p className="text-3xl font-black text-gray-900 tracking-tighter">{shiftStats.itemsSold} <span className="text-lg text-gray-500 font-bold uppercase tracking-widest">Unidades</span></p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <button 
                  onClick={resetShift}
                  className="w-full py-5 bg-orange-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-orange-700 transition-all shadow-lg active:scale-95"
                >
                  Confirmar y Reiniciar Caja
                </button>
                <button 
                  onClick={() => setShowShiftModal(false)}
                  className="w-full py-5 bg-white border-2 border-[#E1E3E5] text-gray-600 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-gray-50 hover:text-gray-900 transition-all active:scale-95"
                >
                  Seguir Vendiendo
                </button>
              </div>
  ...
  return (
    <button 
      ...
      className={`w-full flex items-center gap-4 px-3.5 py-3.5 rounded-xl transition-all ${
        active 
        ? "bg-[#008060] text-white shadow-lg translate-x-1" 
