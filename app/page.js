...
export default function KolmaPOSPremium() {
  ...
  // Cargar Autenticación, Caja Local y Productos al iniciar
  useEffect(() => {
    ...
    // Recuperar caja de ventas anterior (por si recarga la página)
    const savedShift = localStorage.getItem("kolma_shift_stats");
    if (savedShift) {
      try {
        setShiftStats(JSON.parse(savedShift));
      } catch (e) {
    ...
    async function loadProducts() {
      try {
        ...
        if (!res.ok) throw new Error(`Error de conexión con Shopify`);
  ...
  // Lógica de Acceso con contraseña "1221"
  const handleLogin = (e) => {
    e.preventDefault();
  ...
  const handleCheckout = () => {
    ...
    ticket.forEach(item => {
      if (newSoldList[item.id]) {
        ...
        newSoldList[item.id] = { name: item.name, qty: item.qty, total: item.price * item.qty };
      }
    });

    const newStats = {
      totalSales: shiftStats.totalSales + currentTotalMoney,
      itemsSold: shiftStats.itemsSold + currentTotalItems,
      soldProductsList: newSoldList
    };

    setShiftStats(newStats);
    // Guardar en memoria local
    localStorage.setItem("kolma_shift_stats", JSON.stringify(newStats));
  ...
  const generateReportText = () => {
    ...
    if (sortedProducts.length === 0) {
      ...
      sortedProducts.forEach(p => {
        report += `• ${p.name}\n`;
        report += `  Cant: ${p.qty} | Total: RD$${p.total.toLocaleString()}\n\n`;
      });
    }
    return report;
  };
  ...
  const resetShift = () => {
    if(confirm("¿Estás seguro? La caja volverá a cero.")){
      const emptyStats = { totalSales: 0, itemsSold: 0, soldProductsList: {} };
      setShiftStats(emptyStats);
      localStorage.setItem("kolma_shift_stats", JSON.stringify(emptyStats));
  ...
  if (!isAuthChecked || loading) return (
    <div className="h-screen flex items-center justify-center bg-[#F6F6F7]">
      <div className="text-center">
        ...
        <p className="text-[#008060] font-black tracking-widest uppercase text-xs">Cargando Sistema...</p>
      </div>
    </div>
  );
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-[#1A1C1D] text-white font-sans selection:bg-[#008060]">
        <div className="w-full max-w-sm p-8 bg-[#202223] rounded-3xl shadow-2xl border border-gray-800 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 bg-[#008060] rounded-2xl flex items-center justify-center font-black italic text-3xl shadow-lg shadow-emerald-900/50">K</div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="relative">
              ...
              <input 
                ...
                className={`w-full bg-[#1A1C1D] border ${passwordError ? 'border-red-500 text-red-500' : 'border-gray-700 text-white'} rounded-xl py-4 pl-12 pr-4 text-center font-black tracking-[0.5em] focus:outline-none focus:border-[#008060] transition-colors`}
              />
            </div>
            {passwordError && <p className="text-red-500 text-center text-xs font-bold uppercase tracking-widest animate-pulse">Código Incorrecto</p>}
            <button type="submit" className="w-full py-4 bg-[#008060] text-white rounded-xl font-black uppercase tracking-widest hover:bg-[#006e52] transition-colors shadow-lg active:scale-95">
              Desbloquear
            </button>
          </form>
        </div>
  ...
  return (
    <div className="flex h-screen bg-[#F6F6F7] text-[#202223] font-sans overflow-hidden relative">
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[50] lg:hidden transition-opacity" onClick={() => setIsSidebarOpen(false)} />
      )}
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 bg-[#1A1C1D] text-white z-[60] flex flex-col transition-transform duration-300 transform ${isSidebarOpen ? "w-64 translate-x-0" : "w-0 lg:w-20 -translate-x-full lg:translate-x-0 overflow-hidden"}`}>
        ...
        <div className="p-4 border-t border-gray-800 shrink-0 space-y-2">
          ...
          <button 
            ...
            className="flex items-center gap-4 w-full p-3 rounded-xl transition-all font-black text-[10px] uppercase tracking-widest text-gray-500 hover:bg-gray-800 hover:text-white"
          >
            <Icons.Lock />
            {isSidebarOpen && <span>Bloquear</span>}
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-[#E1E3E5] flex items-center px-4 md:px-6 gap-3 md:gap-4 shadow-sm sticky top-0 z-40 shrink-0">
          ...
          <div className="flex flex-col md:flex-row md:items-center bg-emerald-50 text-[#008060] px-3 md:px-4 py-1.5 md:py-2 rounded-xl border border-emerald-100 whitespace-nowrap shadow-sm">
            ...
            <span className="text-xs md:text-sm font-black tracking-tight">RD${shiftStats.totalSales.toLocaleString()}</span>
          </div>
          ...
          <button onClick={() => setIsTicketOpen(true)} className="xl:hidden relative p-2.5 bg-[#008060] text-white rounded-xl shadow-md active:scale-90 transition-transform">
      ...
      {isTicketOpen && <div onClick={() => setIsTicketOpen(false)} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[70] xl:hidden transition-opacity" />}
      ...
      <aside className={`fixed inset-y-0 right-0 w-full sm:w-[400px] lg:w-[420px] xl:static bg-white border-l border-[#E1E3E5] flex flex-col z-[80] xl:z-10 transition-transform duration-300 ease-in-out transform ${isTicketOpen ? "translate-x-0" : "translate-x-full xl:translate-x-0"} shadow-2xl xl:shadow-none`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 custom-scrollbar">
          {ticket.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center scale-90">
              ...
              <p className="font-black text-[10px] uppercase tracking-widest text-gray-500 leading-relaxed">Selecciona productos</p>
            </div>
          ) : ticket.map(item => (
            <div key={item.id} className="flex items-center gap-3 md:gap-4 bg-white border border-[#E1E3E5] p-3 rounded-2xl shadow-sm hover:border-[#008060] transition-colors group">
              <div className="w-12 h-12 bg-[#F9FAFB] rounded-xl p-1 shrink-0 overflow-hidden">
                <img src={item.image} className="w-full h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform" alt="" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between">
                   <div className="flex items-center bg-[#F1F2F3] rounded-lg overflow-hidden border border-[#E1E3E5]">
                    <button onClick={() => updateQty(item.id, -1)} className="w-7 h-7 flex items-center justify-center hover:bg-[#E1E3E5] text-gray-700 font-black">-</button>
              ...
              <button onClick={() => updateQty(item.id, -999)} className="text-gray-300 hover:text-red-500 p-2 rounded-lg hover:bg-red-50"><Icons.Trash /></button>
            </div>
          ))}
        </div>
        ...
        <div className="p-6 bg-[#F9FAFB] border-t border-[#E1E3E5] shrink-0">
          ...
          <button 
            ...
            className={`w-full py-5 rounded-2xl font-black text-xl uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center justify-center gap-2 ${currentTotalMoney > 0 ? "bg-[#008060] text-white hover:bg-[#006e52] shadow-[0_10px_20px_rgba(0,128,96,0.3)]" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          >
            Cerrar Venta
          </button>
        </div>
      ...
      {/* MODAL DETALLADO DE CIERRE DE CAJA */}
      {showShiftModal && (
        <div className="fixed inset-0 bg-[#1A1C1D]/95 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="p-6 md:p-8 pb-4 text-center border-b border-gray-100 shrink-0">
              <h2 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter text-gray-900 mb-1">Corte de Caja</h2>
              <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Resumen y Exportación</p>
            </div>
            
            <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-[#F9FAFB] p-5 rounded-3xl border border-[#E1E3E5] relative overflow-hidden">
              ...
              <div className="mb-6">
                <h3 className="text-xs font-black uppercase text-gray-500 tracking-widest mb-3 border-b pb-2">Top Productos Vendidos</h3>
                <div className="space-y-2">
                  {Object.values(shiftStats.soldProductsList).length === 0 ? (
                    <p className="text-sm text-gray-400 italic">No se han registrado ventas.</p>
                  ) : (
                    Object.values(shiftStats.soldProductsList).sort((a, b) => b.qty - a.qty).map((p, index) => (
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button onClick={handleDownloadReport} disabled={shiftStats.totalSales === 0} className="flex flex-col items-center justify-center p-4 bg-[#F1F2F3] hover:bg-gray-200 text-gray-800 rounded-2xl transition-colors disabled:opacity-50">
                  <Icons.Download />
                  <span className="text-[10px] font-black uppercase mt-2">Descargar Reporte</span>
                </button>
                <button onClick={handleSendWhatsapp} disabled={shiftStats.totalSales === 0} className="flex flex-col items-center justify-center p-4 bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 rounded-2xl transition-colors disabled:opacity-50">
                  <Icons.Whatsapp />
                  <span className="text-[10px] font-black uppercase mt-2">Enviar Whatsapp</span>
                </button>
              </div>

              <div className="space-y-2">
                <button onClick={resetShift} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-black text-sm uppercase tracking-[0.2em] hover:bg-orange-700 shadow-lg active:scale-95">Reiniciar Caja a Cero</button>
                <button onClick={() => setShowShiftModal(false)} className="w-full py-4 bg-white border-2 border-[#E1E3E5] text-gray-600 rounded-2xl font-black text-sm uppercase tracking-[0.2em] hover:bg-gray-50 active:scale-95">Volver al POS</button>
              </div>
            </div>
          </div>
        </div>
...
function MenuBtn({ active, onClick, label, open, icon }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 px-3.5 py-3.5 rounded-xl transition-all ${active ? "bg-[#008060] text-white shadow-[0_8px_16px_rgba(0,128,96,0.25)] translate-x-1" : "text-gray-400 hover:bg-gray-800 hover:text-white"}`}>
